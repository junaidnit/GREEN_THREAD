  ...
  return (
    <div className="mx-auto max-w-[1280px] px-6 py-16 sm:px-10">
      <div className="mb-12 text-center">
        ...
        <h1 className="mx-auto mt-3 max-w-[20ch] font-display text-[clamp(2rem,4.6vw,2.5rem)] leading-tight text-foreground">
          Reading, close to the skin.
        </h1>
        <p className="mx-auto mt-4 max-w-[52ch] text-[16px] font-light leading-relaxed text-muted-foreground">
      ...
      <Link href={lead.href} className="group grid gap-8 border-b border-border pb-14 md:grid-cols-2">
        <div className="relative aspect-[4/3] overflow-hidden bg-surface-2">
          {img(lead.m) && <Image src={sized(img(lead.m), IMG.hero)!} alt={lead.title} fill sizes="(max-width:768px) 100vw, 50vw" priority quality={90} className="object-cover transition-transform duration-700 group-hover:scale-105" />}
        </div>
        <div className="flex flex-col justify-center">
          <span className="eyebrow">{lead.eyebrow}</span>
          <h2 className="mt-3 font-display text-[28px] leading-tight text-foreground group-hover:text-primary">{lead.title}</h2>
          <p className="mt-4 max-w-[46ch] text-[16px] font-light leading-relaxed text-muted-foreground">{lead.blurb}</p>
          <span className="mt-5 text-[12px] font-semibold uppercase tracking-[0.14em] text-slate">Read →</span>
        </div>
      </Link>
      ...
      <div className="mt-14 grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">
        {rest.map((a) => (
          <Link key={a.href} href={a.href} className="group">
            <div className="relative aspect-[4/3] overflow-hidden bg-surface-2">
              {img(a.m) && <Image src={sized(img(a.m), IMG.card)!} alt={a.title} fill sizes="(max-width:768px) 100vw, 33vw" quality={88} className="object-cover transition-transform duration-700 group-hover:scale-105" />}
            </div>
