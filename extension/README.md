# GreenThread — Fabric Check (browser extension)
...
you can buy today. "Buy" always lands on GreenThread, either the exact
...
3. **background.js** posts that scraped text to the GreenThread API
...
4. The server runs the same Claude extraction + scoring rubric the rest of
   GreenThread uses, matches it against the live catalog for a better-fibre
...
3. Click the GreenThread icon in the toolbar → set **API endpoint** to
